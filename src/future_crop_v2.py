...
def get_simple_features(data_dict):
    for variable in ['tas', 'pr', 'rsds', 'tasmax', 'tasmin']:
        ...
        for i in range(fourier_components.shape[1]):
            data_dict[variable][f'fourier_{variable}_{i+1}'] = fourier_components.iloc[:, i]
    ...
    for variable in ['tas', 'pr', 'rsds', 'tasmax', 'tasmin']:
        ...
        for i in range(fourier_components.shape[1]):
            data_dict['features'] = data_dict['features'].join(data_dict[variable][[f'fourier_{variable}_{i+1}']])
